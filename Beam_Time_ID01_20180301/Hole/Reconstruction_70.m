...
I = h5read([p 'Scan0069.cxi'],'/entry_1/instrument_1/detector_1/data');
load('MaxiPix_Mask.mat');
toc;

% Load additional mask due to the hole
mask2 = imread('Mask_70.png');
mask = mask > 0 & mask2 > 128;

% Convert and crop data
I = permute(I,[2 1 3]);
I = single(I(1:512,1:512,1:280));
mask = single(mask(1:512,1:512));
%I = double(I(1:512,1:512,1:280));
%mask = double(mask(1:512,1:512));
...
tic;
[h,dm,ds] = maskNeighbor2(mask,data_input,100);
...
for i = 1:cyc
    % HIO
    [field,object,chi_square_real,chi_square_reci] = cycle(3,iterHIO,beta,epsilon,data_input,support,field,object,mask,chi_square_real,chi_square_reci);
    % ER
    [field,object,chi_square_real,chi_square_reci] = cycle(4,iterER,beta,epsilon,data_input,support,field,object,mask,chi_square_real,chi_square_reci);
    % Shrinkwrap
...
figure(10);
saveas(gcf,['Error_Metric_70_' timestr(3) '_' num2str(round(chi_square_reci(end)),'%.0f') '.png']);
...
save(['Reconstruction_70_' timestr(3) '_' num2str(round(chi_square_reci(end)),'%.0f') '.mat'],...
    'object','object_avg','chi_square_real','chi_square_reci','iterER','iterHIO','cyc','beta','tol','sig0','sigmin','sig','sinit','nx','ny','n_omega');
