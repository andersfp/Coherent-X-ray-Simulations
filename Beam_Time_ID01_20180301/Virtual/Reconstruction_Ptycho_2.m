...
for iii = 1:cyco
    for ii = 1:cyci
        ...
        rho = mean(rhoi.*exp(1i.*2.*pi.*bx./nx.*x).*exp(1i.*2.*pi.*by./ny.*y).*exp(1i.*2.*pi.*bz./n_omega.*z),4);
        % Plots
        a = reshape(chi_square_reci,iterER1,n,ii + (iii - 1)*cyci + cyci*cyco);
        a = permute(a,[1 3 2]);
        a = reshape(a,iterER1*(ii + (iii - 1)*cyci + cyci*cyco),n);
        figure(10);
        semilogy(a);
        figure(100);
        subplot(1,2,1);
    ...
    [bx,by,bz] = phaseSlopeFit(rhoi);
    % Plot alignment
    bxp(:,cyco+iii+1) = squeeze(bx);
    byp(:,cyco+iii+1) = squeeze(by);
    bzp(:,cyco+iii+1) = squeeze(bz);
    figure(20);
    subplot(1,3,1);
    plot(bxp.');
    subplot(1,3,2);
...
save(['Reconstruction_Ptycho_2_' timestr(3) '_' num2str(round(chi_square_reci(end)),'%.0f') '.mat'],...
