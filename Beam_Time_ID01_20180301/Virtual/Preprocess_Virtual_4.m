...
m0 = 10;
for i = 1:n
    w(i) = sum(A(ii(1:m0,i),i));
    z0(i) = sum(squeeze(z(ii(1:m0,i))).*A(ii(1:m0,i),i))./w(i);
end
...
for i = 1:n
    %I(:,:,:,i) = abs(interp3(X,Y,Z,I(:,:,:,i),X,Y,Z + zs(i),'cubic',0)./w(i));
    I(:,:,:,i) = abs(interp3(X,Y,Z,I(:,:,:,i),X,Y,Z + zs(i),'cubic',0));
end


%% Make a rough shift in the xy direction
% Set the shifting amount
sx = [0 0 0 -1 -1 -1 1 1 1]*29; % 29
sy = [0 -1 1 0 -1 1 0 -1 1]*38 + [0 0 0 -1 -1 -1 1 1 1]*2; % 38, 2
...
% for i = 1:m
%     dyx0 = rand(2)*2 - 1;
%     [dyx1(:,:,i)] = dxm2d_findRegistration2(data,scale,dyx0);
% end
% 
% % Determine the shift
% dyx = median(dyx1,3);
...
% % Shift the intensity
% for i = 1:n
%     I(:,:,:,i) = abs(interp3(X,Y,Z,I(:,:,:,i),X + fsx(i),Y + fsy(i),Z,'cubic',0));
%     mask(:,:,:,i) = interp2(X(:,:,1),Y(:,:,1),mask(:,:,:,i),X(:,:,1) + fsx(i),Y(:,:,1) + fsy(i),'linear',0);
%     G(:,:,:,i) = abs(interp2(X(:,:,1),Y(:,:,1),G(:,:,:,i),X(:,:,1) + fsx(i),Y(:,:,1) + fsy(i),'cubic',0));
% end
...
% Fine adjust the scaling
% A = I./G;
% A = sum(sum(A(108:148,108:148,:,:)));
% A = sort(A,3,'descend');
% A = sum(A(:,:,1:10,:),3);
% I = I./A;
...
save('Processed_Virtual_Shifted_4.mat','I','mask','G');
...
save('Processed_Virtual_Stitched_4.mat','I','mask','G');
