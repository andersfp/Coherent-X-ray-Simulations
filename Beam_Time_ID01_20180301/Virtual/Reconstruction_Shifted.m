...
tic;
load('Processed_Virtual_Shifted.mat');
toc;
...
data_input = sqrt(I);
support = ifftshift(fftn(fftshift(sum(data_input,4))));
support = single(abs(support) > sinit*max(abs(support(:))));
field = sum(data_input,4).*exp(1i*2*pi*rand(ny,nx,n_omega));
...
save(['Reconstruction_Shifted_' timestr(3) '_' num2str(round(chi_square_reci(end)),'%.0f') '.mat'],...
