...
I = h5read([p 'Scan0267.cxi'],'/entry_1/instrument_1/detector_1/data'); % 267
...
x = 1:516;
y = x.';
cx = sum(sum(I(:,:,51).*x))./sum(sum(I(:,:,51)));
cy = sum(sum(I(:,:,51).*y))./sum(sum(I(:,:,51)));
ix = round(cx);
iy = round(cy);

% Crop the data
I = I(iy-128:iy+127,ix-128:ix+127,1:100);
mask = mask(iy-128:iy+127,ix-128:ix+127);
...
save(['Reconstruction_Center_' timestr(3) '_' num2str(round(chi_square_reci(end)),'%.0f') '.mat'],...
