...
if sav == 1
    ...
    A.CameraViewAngle = 28;
    axis off;
    set(gcf,'Position',[460 100 1000 800],'Color',[1 1 1]);
    for i = 1:n
        alpha = (i-1)/n*360;
        A.CameraPosition = [-sind(alpha)*1000 500 cosd(alpha)*1000];
        drawnow;
        mov(i) = getframe(gcf);
    end
    vid = VideoWriter('NoLens_Center.mp4','MPEG-4');
    vid.FrameRate = 60;
    open(vid);
    writeVideo(vid,mov);
    close(vid);
...
B = field(:,:,141);
dphix = diff(angle(B),1,2);
dphix(dphix > pi) = dphix(dphix > pi) - 2*pi;
dphix(dphix < -pi) = dphix(dphix < -pi) + 2*pi;
dphiy = diff(angle(B),1,1);
dphiy(dphiy > pi) = dphiy(dphiy > pi) - 2*pi;
dphiy(dphiy < -pi) = dphiy(dphiy < -pi) + 2*pi;
Px = dphix(1:end-1,:).*log10(abs(B(1:end-1,1:end-1)));
Py = dphiy(:,1:end-1).*log10(abs(B(1:end-1,1:end-1)));
...
figure;
subplot(1,2,1);
imagesc(complex2rgb(A(:,:,141),cmap));
axis equal tight;
subplot(1,2,2);
quiver(Px,Py);
axis equal tight;
...
save('Corrected_NoLens_Center.mat','object_avg','rx','ry','rz');
