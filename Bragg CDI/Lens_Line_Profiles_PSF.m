...
vx1 = PSF_Fitting(x,fx1,1./efox,2*510,sig0);
figure;
semilogy(qx,[fx1 MTF_Fitting(x,[w0;2*510;vx1])]);

vxa1 = PSF_Fitting(x,fxa1,1./efox,2*510,sig0);
figure;
semilogy(qx,[fxa1 MTF_Fitting(x,[w0;2*510;vxa1])]);

vx2 = PSF_Fitting(x,fx2,1./efox,2*510,sig0);
figure;
semilogy(qx,[fx2 MTF_Fitting(x,[w0;2*510;vx2])]);

vxa2 = PSF_Fitting(x,fxa2,1./efox,2*510,sig0);
figure;
semilogy(qx,[fxa2 MTF_Fitting(x,[w0;2*510;vxa2])]);

vx = [vx1 vxa1 vx2 vxa2];

vy1 = PSF_Fitting(y,fy1,1./efoy,2*510,sig0);
figure;
semilogy(qy,[fy1 MTF_Fitting(y,[w0;2*510;vy1])]);

vya1 = PSF_Fitting(y,fya1,1./efoy,2*510,sig0);
figure;
semilogy(qy,[fya1 MTF_Fitting(y,[w0;2*510;vya1])]);

vy2 = PSF_Fitting(y,fy2,1./efoy,2*510,sig0);
figure;
semilogy(qy,[fy2 MTF_Fitting(y,[w0;2*510;vy2])]);

vya2 = PSF_Fitting(y,fya2,1./efoy,2*510,sig0);
figure;
semilogy(qy,[fya2 MTF_Fitting(y,[w0;2*510;vya2])]);

vy = [vy1 vya1 vy2 vya2];

vz1 = PSF_Fitting(z,fz1,1./efoz.*(abs(qz) < 0.04),2*600,sig0);
figure;
semilogy(qz,[fz1 MTF_Fitting(z,[w0;2*600;vz1])]);

vza1 = PSF_Fitting(z,fza1,1./efoz.*(abs(qz) < 0.04),2*z(311),sig0);
figure;
semilogy(qz,[fza1 MTF_Fitting(z,[w0;2*z(311);vza1])]);

vz2 = PSF_Fitting(z,fz2,1./efoz.*(abs(qz) < 0.1),2*600,sig0);
figure;
semilogy(qz,[fz2 MTF_Fitting(z,[w0;2*600;vz2])]);

vza2 = PSF_Fitting(z,fza2,1./efoz.*(abs(qz) < 0.014),2*530,sig0);
figure;
semilogy(qz,[fza2 MTF_Fitting(z,[w0;2*530;vza2])]);
