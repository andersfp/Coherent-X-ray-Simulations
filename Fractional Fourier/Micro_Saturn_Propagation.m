...
tic;
for i = 1:N
    [EN,xN,yN] = propFrFT2(EN,xN,yN,Inf,Inf,[smx(i) smy(i)],[spx(i) spy(i)],[ax(i) ay(i)],lambda,'gpu');
    EN = EN.*att(xN,yN);
end
toc;
IN = abs(EN).^2;
...
tic;
[EM,xM,yM] = propFrFT2(EN,xN,yN,Inf,Inf,[smx(end) smy(end)],[spx(end) spy(end)],[ax(end) ay(end)],lambda,'gpu');
toc;
IM = abs(EM).^2;

% Propagate in 1 step
tic;
[E1,x1,y1] = propFrFT2(E0,x0,y0,Inf,Inf,[smx(1) smy(1)],[spx(end-1) spy(end-1)],[sum(ax(1:end-1)) sum(ay(1:end-1))],lambda,'gpu');
toc;
I1 = abs(E1).^2;
...
ff = fit([X(:),Y(:)],Z(:),fun,'StartPoint',sp,'Exclude',Z(:) > 1,'Robust','Bisquare');

% Propagate to the image plane
att = sqrt(exp(-(x1.^2 + y1.^2)./(2*(1e-4*ff.sig)^2)));
tic;
[E2,x2,y2] = propFrFT2(E1.*att,x1,y1,Inf,Inf,[smx(end) smy(end)],[spx(end) spy(end)],[ax(end) ay(end)],lambda,'gpu');
toc;
I2 = abs(E2).^2;
...
if s == 1
    tic;
    %save([p 'Micro_Saturn_Propagation.mat'],'-v7.3');
    clear E0 E1 E2 EN EM X Y Z sat att
    save('Micro_Saturn_Propagation.mat','-v7.3');
    toc;
end
