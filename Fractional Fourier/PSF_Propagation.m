...
yN = abs(LN).^2;
ff = fit(xN,yN,fun,'StartPoint',[yN(m/2+1) sqrt(sum(xN.^2.*yN)./sum(yN))]);
...
% Propagate to the image plane
att = sqrt(exp(-(x1.^2 + x1.'.^2)./(2*(ff.sig)^2)));
tic;
...
if s == 1
    tic;
    %save([p 'PSF_Propagation.mat'],'E0','x0','L0','E1','x1','L1','E2','x2','L2','EN','xN','LN','EM','xM','LM','-v7.3');
    clear E0 E1 E2 EN EM I0 I1 I2 IN IM;
    save('PSF_Propagation_Profiles.mat','-v7.3');
    toc;
end
