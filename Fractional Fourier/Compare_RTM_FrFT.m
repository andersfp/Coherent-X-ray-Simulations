...
% Test the position sensitivity of d2 by sum(a) and Rp(end)
dd = linspace(-1e-6,1e-6,201);
n = length(dd);
A2 = zeros(n,1);
R3 = A2;
for i = 1:n
    Dt = D + [zeros(N,1);dd(i)];
    [A,~,R2,~,~,~,~] = FrFT_parameters(Dt,F,lambda,R0,s0);
    A2(i) = sum(A);
    R3(i) = R2(end);
end
figure;
plotyy(dd,A2-2,dd,R3);
...
subplot(2,2,1);
plot(x2,l2,'-',x70,l70,'--');
set(gca,'XLim',[-1.5e-6 1.5e-6],'YLim',[0 1]);
...
yyaxis left;
plot(x2,l2 - l70);
set(gca,'XLim',[-1.5e-6 1.5e-6],'YLim',[-2e-5 2e-5]);
title('Detector intensity error');
xlabel('x [m]');
ylabel('Absolute error [a.u.]');
yyaxis right;
plot(x2,(l2 - l70)./l70);
set(gca,'XLim',[-1.5e-6 1.5e-6],'YLim',[-1e-3 1e-3]);
ylabel('Relative error');
subplot(2,2,3);
plot(x2,p2,'-',x70,p70,'--');
set(gca,'XLim',[-1.5e-6 1.5e-6],'YLim',[0 0.03]);
...
yyaxis left;
plot(x2,p2 - p70);
set(gca,'XLim',[-1.5e-6 1.5e-6],'YLim',[-3e-4 3e-4]);
title('Detector phase error');
xlabel('x [m]');
ylabel('Absolute error [rad]');
yyaxis right;
plot(x2,(p2 - p70)./p70);
set(gca,'XLim',[-1.5e-6 1.5e-6],'YLim',[-0.02 0.02]);
ylabel('Relative error');
