...
I1 = abs(E1).^2;
if s == 1
    Save_Tiff([p 'Effective_Aperture_Large_I1.tiff'],I1,max(max(I1)));
end

% Propagate to the image plane
att = sqrt(exp(-(x1.^2 + x1.'.^2)/(2*(8.293e-5)^2)));
tic;
[E2,~,x2] = propFrFFT(E1.*att,x1,x1,Rm(end),Rp(end),sm(end),sp(end),a(end),lambda,'gpu');
toc;
L2 = E2(:,m/2+1);
I2 = abs(E2).^2;
if s == 1
    Save_Tiff([p 'Effective_Aperture_Large_I2.tiff'],I2,max(max(I2)));
end
...
IN = abs(EN).^2;
if s == 1
    Save_Tiff([p 'Effective_Aperture_Large_IN.tiff'],IN,max(max(IN)));
end
...
IM = abs(EM).^2;
if s == 1
    Save_Tiff([p 'Effective_Aperture_Large_IM.tiff'],IM,max(max(IM)));
end

% Save the electric field on the CRL entrance
if s == 1
    tic;
    save([p 'Effective_Aperture_Large.mat'],'E0','x0','L0','E1','x1','L1','E2','x2','L2','EN','xN','LN','EM','xM','LM','-v7.3');
    save('Effective_Aperture_Large_Profiles.mat','L0','x0','L1','x1','L2','x2','LN','xN','LM','xM','-v7.3');
    toc;
end
